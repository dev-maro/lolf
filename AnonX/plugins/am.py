...
@app.on_message(filters.command("command") & filters.private, group=1)
...
@app.on_message(filters.regex("^كتم$") & filters.group)
async def mute(app,message):
   member = await message.chat.get_member(message.from_user.id)
   if not member.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
     return await message.reply("**فقط الادمن يستطيع استخدام الامر**")
   else:
     if not message.reply_to_message:
       return await message.reply("**• اعمل ريبلاي عليه**")
     member = await message.chat.get_member(message.reply_to_message.from_user.id)
     tala = await message.chat.get_member(DEVSOURCE)
     if member.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
       return await message.reply("**عايز تكتم.الادمن احا 🙂**")
     if tala.status in DEVSOURCE:
       return await message.reply("**دا مطور السورس عايز تكتمو لول**")       
     chat_id = str(message.chat.id)
     user_id = str(message.reply_to_message.from_user.id)
     x = "{}@{}".format(chat_id,user_id)
     if x in mutes:
       return await message.reply("**يا قلبي والله هو مكتوم**")
     else:
       mutes.append(x)
       return await message.reply_animation("https://t.me/anmii288/8",
                                    caption="**◍ المستخدم {}\n◍ تم كتمه من قبل {} **".format(message.reply_to_message.from_user.mention,message.from_user.mention))
       
@app.on_message(filters.regex("^الغاء كتم$") & filters.group)
async def unmute(app,message):
   member = await message.chat.get_member(message.from_user.id)
   if not member.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
     return await message.reply("**بتعمل اي يا بيه الادمن بس يقدر يستخدم الامر**")
   else:
     if not message.reply_to_message:
       return await message.reply("**• اعمل ريبلاي عليه علشان تفك الكتم**")
     member = await message.chat.get_member(message.reply_to_message.from_user.id)
     if member.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
       return await message.reply("**عايز تكتم الادمن احا 🙂**")
     chat_id = str(message.chat.id)
     ...
     x = "{}@{}".format(chat_id,user_id)
     if not x in mutes:
       return await message.reply("**يا باشا هو مش مكتوم والله **")
     else:
       mutes.remove(x)
       return await message.reply_animation("https://t.me/anmii288/9",
                                    caption="**◍ المستخدم {}\n◍ تم الغاء كتمه من قبل {} **".format(message.reply_to_message.from_user.mention,message.from_user.mention))

@app.on_message(filters.regex("^المكتومين$") & filters.group)
def get_dmute(app, message):
   ...
   ch = message.chat.id
   c = 0
   text = "• اليك يا عزيزي قائمه المكتومين :\n\n"
   for a in mutes:
     ...
     if chat_id == ch:
        ...
        text += f"- {user.mention}\n"
        c += 1
   if c == 0: return message.reply("**مفيش حد مكتوم يا قلب**")
